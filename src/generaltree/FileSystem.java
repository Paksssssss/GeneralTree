        ...
        if (path.contains("/")) {
            ...
            if (tempNode != null) {
                if (!tempNode.getFileDescriptor().isDir) {
                    ...
                    textAreaLogs = textArea.getText();
                    textArea.setText(tempNode.getContent());
                    textArea.setEditable(true);
                    textArea.requestFocus();
                    JTextField tempField = textField;
                    textField.setNavigationFilter(new NavigationFilter());
                    textField.setText("");
                    textField.removeActionListener(action);
                    editorAction = new AbstractAction() {
                        @Override
                        public void actionPerformed(ActionEvent e) {
                            String input[] = textField.getText().split("\\s+");
                            String command;
                            String actualInput;
                            if (input[0].isEmpty() && input.length == 3) {
                                command = input[1].replaceAll("\\s+", "");
                                actualInput = input[2];
                            } else if (input.length == 1) {
                                actualInput = "";
                                command = input[0].replaceAll("\\s+", "");
                            } else {
                                command = input[0].replaceAll("\\s+", "");
                                actualInput = input[1];
                            }
                            ...
                            if (command.equals(":q")) {
                                textArea.setText(textAreaLogs);
                                textArea.setEditable(false);
                                textField.setText(system.pathToCurrent + ">");
                                textField = tempField;
                            } else if (command.equals(":w")) { //writes or overwrites
                                if (actualInput.equals(system.currentNode.getShortName())) {
                                    system.currentNode.setContent(textArea.getText());
                                    system.currentNode.getFileDescriptor().setDate();
                                } else if (actualInput.isEmpty()) {
                                    system.currentNode.setContent(textArea.getText());
                                    system.currentNode.getFileDescriptor().setDate();
                                } else {
                                    TreeNode newFile = new TreeNode(actualInput, false);
                                    newFile.getFileDescriptor().setDate();
                                    if (actualInput.contains(".")) {
                                        newFile.setFileDescriptor(new Descriptor(actualInput.split(".")[0], actualInput.split(".")[1]));
                                    } else {
                                        newFile.setFileDescriptor(new Descriptor(actualInput, ""));
                                    }

                                    newFile.setParent(system.currentNode.getParent());
                                    system.currentNode.getChildren().add(newFile);
                                }
                                textArea.setText(textAreaLogs);
                                textArea.setEditable(false);
                                textField.setText(system.pathToCurrent + ">");
                    ...
                    textField.addActionListener(editorAction);
                    system.currentNode = tempNode;
                } else {
                    textArea.setText(textArea.getText().concat(">Specified path is a Directory\n"));
                }
            } else {
                tempNode = system.goToPath(tempPath);
                if (tempNode != null) {
                            ...
                            if (command.equals(":q")) {
                                ...
                                if (actualInput.equals(system.currentNode.getShortName())) {
                                    ...
                                    if (fileName.contains(".")) {
                                        newFile.setFileDescriptor(new Descriptor(fileName.split(".")[0], fileName.split(".")[1]));
                                    } else {
                                        newFile.setFileDescriptor(new Descriptor(fileName, ""));
                                    }
                                    newFile.setParent(system.currentNode.getParent());
                                    system.currentNode.getChildren().add(newFile);
                                } else {
                                    ...
                                    newFile.setParent(system.currentNode);
                                    system.insert(newFile);
                                }
                                textArea.setText(textAreaLogs);
                                textArea.setEditable(false);
                                textField.setText(system.pathToCurrent + ">");
                ...
                editorAction = new AbstractAction() {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        ...
                        if (command.equals(":q")) {
                            ...
                            if (actualInput.equals(fileName)) {
                                system.currentNode.setContent(textArea.getText());
                                system.currentNode.getFileDescriptor().setDate();
                            } else if (actualInput.isEmpty()) {
                                TreeNode newFile = new TreeNode(actualInput, false);
                                newFile.getFileDescriptor().setDate();
                                if (path.contains(".")) {
                                    newFile.setFileDescriptor(new Descriptor(path.split(".")[0], path.split(".")[1]));
                                } else {
                                    newFile.setFileDescriptor(new Descriptor(path, ""));
                                }
                                newFile.setParent(system.currentNode);
                                system.insert(newFile);
                            } else {
